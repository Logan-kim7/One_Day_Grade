...
import com.biz.grade.service.ScoerService;
import com.biz.grade.service.ScoerServiceImplV1;
...
public class GradeEx_01 {
	...
	public static void main(String[] args) {
		...
		ScoerService scService = new ScoerServiceImplV1();
		scService.loadScore();
		...
		while (true) {
			...
			try {
				...
				System.out.println("메뉴는 숫자로만 선택해주세요");
				continue;
			}
			if (intMenu == DBcontract.MENU.학생정보등록) {
				...
				// 3. inputScore()가 false를 return 하면 반복을 중단한다.
				while (scService.inputScore())
					;

				// 성적입력 종료(중단) 했을경우 총점과 평균 계산 method 호출
				scService.calcSum();
				scService.calcAvg();
